...
namespace SPN {
    ...
    void OnlineExpectMax::fit(std::vector<std::vector<double>> &trains,
        ...
        for (size_t t = 0; t < num_iters; ++t) {
            ...
            std::shuffle(valids.begin(), valids.end(), g);
            int train_subsamp = 1000;
            // int val_subsamp = 100;
            // Record current training and validation set log-likelihoods.
            for (size_t n = 0; n < train_subsamp + num_valids; ++n) {
                if (n < train_subsamp) {
                    train_logps += spn.EvalDiff(trains[n], mask_false);
                } else {
                    valid_logps += spn.EvalDiff(valids[n - train_subsamp], mask_false);
                }
            }
            train_logps /= train_subsamp;
            ...
            std::stringstream ss;
            ss << "online_weights_t_" << t << ".mdl";
            SPN::utils::save( &spn, ss.str());
