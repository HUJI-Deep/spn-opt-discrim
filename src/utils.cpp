...
namespace SPN {
    namespace utils {
            ...
            for (SPNNode *pt : spn->top_down_order()) {
                if (pt->type() == SPNNodeType::SUMNODE) {
                    ...
                    if (((VarNode *)pt)->distribution() == VarNodeType::NORMALNODE) {
                        fout << pt->id() << ",NORMALNODE," << pt->scope()[0] << ",";
                        fout << ((NormalNode *)pt)->var_mean() << ","
                             << ((NormalNode *)pt)->var_var() << "\n";
                    }
        ...
        SPNetwork *load(std::string filename) {
            ...
            for (const auto &kv : id2node) {
                if (kv.second->num_parents() == 0){
                    ...
                    std::cout << "root: " << kv.first << std::endl;
                }
        ...
        std::vector<std::vector<bool>> load_masks(std::string filename) {
            double eps = 0.0001;
            double db;
            std::ifstream fin(filename, std::ifstream::in);
            if (!fin) {
