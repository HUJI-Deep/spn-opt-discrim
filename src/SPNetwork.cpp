...
namespace SPN {
    ...
    std::vector<int> SPNetwork::batchClassify(const std::vector<std::vector<double>> &inputs, bool verbose) {
        size_t num_inputs = inputs.size();
        std::vector<bool> mask(inputs[0].size(), false);
        std::vector<int> preds(num_inputs);
        
        for (size_t n = 0; n < num_inputs; ++n) {
            preds[n] = Classify(inputs[n], mask);
            
        }
        return preds;
    }

    std::vector<int> SPNetwork::batchClassify(const std::vector<std::vector<double>> &inputs,const std::vector<std::vector<bool>> &masks, bool verbose) {
        size_t num_inputs = inputs.size();
        std::vector<int> preds(num_inputs);
        
        for (size_t n = 0; n < num_inputs; ++n) {
            preds[n] = Classify(inputs[n], masks[n]);
            
        }
        return preds;
    }

    int SPNetwork::Classify(const std::vector<double> &input, const std::vector<bool> &mask) {
        // std::cout << "in classify"<<std::endl;
        int K = 10;
        int argmax = -1;
        int LABEL_VAR_START_DIM = 784;
        double res=1, max=-std::numeric_limits<double>::infinity();
        std::vector<double> outputs(K);
        std::vector<double> mod_input(LABEL_VAR_START_DIM + K);

        for (int i=0; i < input.size() ; i++){

            if (i < LABEL_VAR_START_DIM) {
                mod_input[i] = input[i];
            } else {
                mod_input[i] = 0;
            }
            // std::cout << "input["<<i<<"]="<<mod_input[i] << " ";
        ...
        for(int k=0; k < K; k++){
            mod_input[LABEL_VAR_START_DIM + k] = 1;
            res = Eval(mod_input, mask);
            if (res > max){
                argmax = k;
                max = res;
            ...
            outputs[k] = res;
            mod_input[LABEL_VAR_START_DIM + k] = 0;
        }
    ...
    double SPNetwork::Eval(const std::vector<double> &input, const std::vector<bool> &mask) {
        ...
        for (VarNode *pt : dist_nodes_) {
            ...
            if (mask[var_index]) {
                ...
                //std::cout << "input["<< var_index <<"] = " << input[var_index] << std::endl ;
            }
        }
